			...
			public void onTick(
				...
				if (tick % 500 == 0) {
					...
					for (var sub : submarines) {
						...
						sb.append(String.format("  sub%d: [%.0f,%.0f,%.0f] spd=%.1f hp=%d noise=%.1f %s", sub.id(),
								pos.x(), pos.y(), pos.z(), sub.speed(), sub.hp(), sub.noiseLevel(),
								sub.status() != null ? sub.status() : ""));
						// Log active waypoint and contact estimate
						var wps = sub.waypoints();
						if (wps != null) {
							var active = wps.stream().filter(se.hirt.searobots.api.Waypoint::active).findFirst();
							active.ifPresent(wp -> sb.append(String.format(" wp:[%.0f,%.0f]", wp.x(), wp.y())));
						}
						var contacts = sub.contactEstimates();
						if (contacts != null && !contacts.isEmpty()) {
							var ce = contacts.getFirst();
							sb.append(String.format(" ce:[%.0f,%.0f,a=%.2f,hdg=%.0f,spd=%.1f]", ce.x(), ce.y(),
									ce.contactAlive(),
